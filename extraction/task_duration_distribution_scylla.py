...
def dist_best(series, graph, bins):
	...
	if(graph==True):
		...
		best_fit_name, best_fir_paramms = best_fit_distribution(data, bins)
...
def best_fit_distribution(data, bins=50, ax=None):
	...
	DISTRIBUTIONS = [st.uniform,st.norm,st.triang,st.expon]
	# Best holders
...
def get_task_distribution(task_data, graph=False, bins=200):
	...
	if not task_data:
		...
		if np.min(task_data)==np.max(task_data):
			...
			dname = dist_best(task_data, graph, bins)
			dparams =  dist_params(dname, task_data)
	return dict(dname=dist_names(dname),dparams=dparams)
