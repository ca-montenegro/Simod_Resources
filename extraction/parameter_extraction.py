...
def extract_parameters(log, bpmn, process_graph,flag,k,sim_percentage,simulator,quantity_by_cost,reverse_cost, happy_path=False):
    if bpmn != None and log != None:
        ...
        #-------------------------------------------------------------------
        # Process replaying
        conformed_traces, not_conformed_traces, process_stats = rpl.replay(process_graph, log,resource_table=resource_table)
        # -------------------------------------------------------------------
                ...
                if (sim == 'bimp'):
                    ...
                    for key2, group2 in itertools.groupby(role_sorted, key=lambda x: x['role']):
                        ...
                        if len(group_count) > max_count:
                            max_count = len(group_count)
                            max_role = key2
                    elements_data_scylla.append(dict(id=sup.gen_id(), elementid=task_id, type=dist_scylla['dname'], name=task_name,
        ...
        for sim in simulator:
            if sim=='bimp':
                parameters_bimp = dict(arrival_rate=arrival_rate_bimp, time_table=time_table, resource_pool=resource_pool,
                ...
                parameters_scylla = dict(arrival_rate=arrival_rate_scylla, time_table=time_table, resource_pool=resource_pool,
                                       elements_data=elements_data_scylla, sequences=sequences, instances=len(conformed_traces),
                                       bpmnId=bpmnId, resource_table=resource_table, roles=roles, flag=flag)
